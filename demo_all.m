...
imgRoot='./MSRA-1000/';% test image path
saldir='./DSR_MSRA-1000map/';% the output path of the saliency map
...
outname=[saldir imnames(ii).name(1:end-4)  '_MSE.bmp'];   
...
outname=[saldir imnames(ii).name(1:end-4)  '_MSEP.bmp'];   
imwrite(propSRRecErrorSaliency,outname);
smap = t5 ./ t1;
...
outname=[saldir imnames(ii).name(1:end-4)  '_MSEPG.bmp'];   
imwrite(propSRRecErrorSaliency,outname);
...
outname=[saldir imnames(ii).name(1:end-4)  '_DSR.bmp'];   
imwrite(saliency,outname);
% imwrite(saliency,strcat(saldir,imName(1:end-4),'_DSR.bmp'));
end
toc;